/**
 * Tests for the bot-at-bot identity & guidance builders in
 * dispatch-builders.ts:
 *  - buildFeishuIdentityFields  → SenderIsBot / BotOpenId injection
 *  - buildFeishuGroupSystemPrompt → self-identity + @ rule guidance
 */
...
describe('buildFeishuGroupSystemPrompt', () => {
  it('injects self open_id and the @-delivery rule', () => {
  ...
  it('appends the operator-configured group prompt after the guidance', () => {
    ...
    // guidance (self identity) comes before the configured prompt
    expect(prompt.indexOf('ou_self')).toBeLessThan(prompt.indexOf('Be concise.'));
  });

  it('still injects guidance even with no configured prompt and no open_id', () => {
